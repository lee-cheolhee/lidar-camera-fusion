...
// input topics 
std::string imgTopic = "/zed2i/zed_node/left/image_rect_color";
...
void callback(const boost::shared_ptr<const sensor_msgs::PointCloud2>& in_pc2 , const ImageConstPtr& in_image)
{
    cv_bridge::CvImagePtr cv_ptr , color_pcl;
        try
        {
//			cv_image = cv_bridge::toCvCopy(image_msg_, "bgra8");
//			cv_ptr = cv_bridge::toCvCopy(in_image, sensor_msgs::image_encodings::BGRA8);
			cv_ptr = cv_bridge::toCvCopy(in_image, "bgra8");
//		    color_pcl = cv_bridge::toCvCopy(in_image, sensor_msgs::image_encodings::BGR8);
			color_pcl = cv_bridge::toCvCopy(in_image, "bgra8");
		}
