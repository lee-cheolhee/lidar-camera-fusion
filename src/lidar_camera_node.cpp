...
void callback(const boost::shared_ptr<const sensor_msgs::PointCloud2>& in_pc2_msg, const ImageConstPtr& in_image_msg)
{
	cv_bridge::CvImagePtr cv_ptr , color_pcl;
	try {
		// camera -> lidar
//		cv_ptr = cv_bridge::toCvCopy(in_image_msg, sensor_msgs::image_encodings::BGR8);
		cv_ptr = cv_bridge::toCvCopy(in_image_msg, "bgra8");
		// lidar -> camera
//		color_pcl = cv_bridge::toCvCopy(in_image_msg, sensor_msgs::image_encodings::BGR8);
		color_pcl = cv_bridge::toCvCopy(in_image_msg, "bgra8");
	}
	...
	pcl_conversions::toPCL(*in_pc2_msg, pcl_pc2);		// pc2�޼����� pcl�� ��ȯ
...
//				varianza = sqrt(varianza / interpol_value);
	...
	unsigned int cols = in_image_msg->width;
	unsigned int rows = in_image_msg->height;
...
//		pc_matrix(3,0) = 1.0;
		...
		// TODO: 이해가 잘 안됨 Tlc를 빼줘야 할 것 같은데, 더해줘야지 정확한 퓨전이 됨
		pc_matrix(0,0) = P_out->points[i].x + Tlc(0);
		pc_matrix(1,0) = P_out->points[i].y + Tlc(1);
		pc_matrix(2,0) = P_out->points[i].z + Tlc(2);
		pc_matrix(3,0) = 1.0;
		...
		cv::Vec4b & color = color_pcl->image.at<cv::Vec4b>(py_data, px_data);
